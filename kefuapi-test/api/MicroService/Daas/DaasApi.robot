...
/daas/internal/agent/kpi/wl
    [Arguments]    ${agent}    ${timeout}    ${DateRange}    ${FilterEntity}
    ${header}=    Create Dictionary    tenantId="${agent.tenantId}"    SESSION=${agent.session}    userid=${agent.userId}
    ${params}    set variable    beginDateTime=${DateRange.beginDateTime}&endDateTime=${DateRange.endDateTime}&dataAuth=${FilterEntity.dataAuth}&page=${FilterEntity.page}&pageSize=${FilterEntity.per_page}&agentId=${agent.userId}&sessionType=${FilterEntity.sessionType}
...
/daas/internal/group/kpi/wl
    [Arguments]    ${agent}    ${timeout}    ${DateRange}    ${FilterEntity}
    ${header}=    Create Dictionary    tenantId="${agent.tenantId}"    SESSION=${agent.session}    userid=${agent.userId}
    ${params}    set variable    beginDateTime=${DateRange.beginDateTime}&endDateTime=${DateRange.endDateTime}&dataAuth=${FilterEntity.dataAuth}&page=${FilterEntity.page}&pageSize=${FilterEntity.per_page}&groupId=${FilterEntity.groupId}

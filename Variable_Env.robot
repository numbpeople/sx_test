...
&{requestHeader}    Content-Type=    Authorization=
...
...               validAppUUID=    # 有效的app和user均在validOrgName组织下，无效变量则随机取值
${allowOpenRegistration}    true    # 应用APP开放注册（true）、授权注册（false）
&{RunModelCaseConditionDic}    orgName=    appName=    specificBestToken=    specificAppkey=    # 是否指定appkey、是否指定超级token

...
获取不存在的org下的应用app列表(/management/organizations/{orgName}/applications)
    ...
    ${contentType.JSON}    ${Token.orgToken}    ${UnableToAuthenticateDictionary.statusCode}    ${UnableToAuthenticateDictionary.reponseResult}    ${UnableToAuthenticateDiffEntity}    ${ModelCaseRunStatus.OrgToken_ContentType}
    ${contentType.JSON}    ${EMPTY}    ${EasemobSecurityExceptionDictionary.statusCode}    ${EasemobSecurityExceptionDictionary.reponseResult}    ${EasemobSecurityExceptionDiffEntity}    ${ModelCaseRunStatus.EmptyOrgToken_ContentType}
    ${EMPTY}    ${Token.orgToken}    ${UnableToAuthenticateDictionary.statusCode}    ${UnableToAuthenticateDictionary.reponseResult}    ${UnableToAuthenticateDiffEntity}    ${ModelCaseRunStatus.OrgToken_EmptyContentType}
    ${EMPTY}    ${EMPTY}    ${EasemobSecurityExceptionDictionary.statusCode}    ${EasemobSecurityExceptionDictionary.reponseResult}    ${EasemobSecurityExceptionDiffEntity}    ${ModelCaseRunStatus.EmptyOrgToken_EmptyContentType}
    ${contentType.JSON}    ${Token.bestToken}    ${UnableToAuthenticateDictionary.statusCode}    ${UnableToAuthenticateDictionary.reponseResult}    ${UnableToAuthenticateDiffEntity}    False    #该条case不执行了，因为有超管token条件下，接口返回值会不同

...
Send SecondGateway Msg
    ...
    #拼接消息中的weichatExt字段
    ${ext}    Message Ext    ${msg}    ${msg_id_for_ack}
    #避免重复key，先移除ext字段
...
Message Ext
    [Arguments]    ${msg}    ${msg_id_for_ack}
    [Documentation]    拼接消息中的weichatExt字段
    #当消息的ext为空或为{}或不存在,则重新设置ext
    ${ExtExist}    Run Keyword And Return Status    Dictionary Should Not Contain Key    ${msg}    ext
    ${status}    Run Keyword And Return Status    should be equal    ${msg.ext}    {}
    ${NonExt}    Run Keyword And Return Status    should be equal    ${msg.ext}    ${EMPTY}
    Run Keyword And Return If    ${ExtExist} or ${status} or ${NonExt}    Non Message Ext    ${msg_id_for_ack}
    #处理ext字段
    ...
    Return From Keyword    ${ext}

Non Message Ext
    [Arguments]    ${msg_id_for_ack}
    [Documentation]    当消息的weichatExt为空或为{},则重新设置weichatExt
    ${weichatExt}    set variable    {"msg_id_for_ack": "${msg_id_for_ack}"}    #重新设置weichat扩展
    Return From Keyword    ${weichatExt}

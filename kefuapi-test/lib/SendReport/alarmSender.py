...
class SMS_EMAIL_Receiver:
    ...
    # 独立模块 监控
    mod_messages =  [k for k, v in PhoneEmailNumber.items()]

    mod_robot = QA_TEAM
    mod_callback = QA_TEAM
    mod_statis = QA_TEAM
    mod_gateway = QA_TEAM
    mod_adminserver = QA_TEAM
    ...
    def sendReportMail(self, sub, msg, to, attachment_data=None):
        ...
        html = u"""\
            <html>
              ...
              <body>
                <p>Kefuapi_ci test failed, Detail:</p>
                <a class="mnav"></a>
                <p>%s</p>
...
if __name__ == '__main__':
    s = AlarmSender()
    # s.send_sms('test')
    s.sendReportMail('test', 'this is test', ['leoli@easemob.com'])

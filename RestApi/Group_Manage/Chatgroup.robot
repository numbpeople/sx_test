*** Variables ***
&{pubgroup_notvalidate}    groupname=public_group1    desc=RST_not_validate    public=${true}    maxusers=1000    approval=${false}    owner=    members=
&{pubgroup_validate}    groupname=public_group2    desc=RST_need_owner_validate    public=${true}    maxusers=1000    approval=${true}    owner=    members=
&{private_invite}    groupname=private_group    desc=RST_private_allow_member_invite    public=${false}    maxusers=800    allowinvites=${true}    owner=    members=
&{private_not_invite}    groupname=private_group    desc=RST_private_not_invite    public=${false}    maxusers=800    allowinvites=${false}    owner=    members=
&{transfer_group}    newowner=
&{modify_info}    groupname=    description=    maxusers=
&{add_admin}      newadmin=
&{add_multi_member}    usernames=
&{apply_verify}    applicant=    verifyResult=true
&{mute_body}      usernames=    mute_duration=${86400000}

*** Keywords ***
/{org_name}/{app_name}/chatgroups?limit=8
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${header}    ${timeout}
    [Documentation]    分页获取APP下的群组
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups?limit=8
    Run Keyword And Return    Get Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups
    [Arguments]    ${method}    ${session}    ${org_name}    ${app_name}    ${header}    ${data}
    ...    ${timeout}
    [Documentation]    1、不分页获取APP下的群组
    ...    2、创建群组
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups
    Run Keyword And Return if    ${method}=='get'    Get Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}
    Run Keyword And Return if    ${method}=='post'    Post Request    ${session}    ${uri}    headers=${header}    data=${data}
    ...    timeout=${timeout}

/{org_name}/{app_name}/publicchatgroups?limit=6
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${header}    ${timeout}
    [Documentation]    分页获取APP下的公开群组
    ${uri}=    set variable    /${org_name}/${app_name}/publicchatgroups?limit=6
    Run Keyword And Return    Get Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/publicchatgroups
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${header}    ${timeout}
    [Documentation]    不分页获取APP下的公开群组
    ${uri}=    set variable    /${org_name}/${app_name}/publicchatgroups
    Run Keyword And Return    Get Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/users/{user}/joined_chatgroups
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${user}    ${header}    ${timeout}
    [Documentation]    获取IM用户加入的所有群组
    ${uri}=    set variable    /${org_name}/${app_name}/users/${user}/joined_chatgroups
    Run Keyword And Return    Get Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid1}{groupid2}{groupid3}{groupid4}
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${groupid1}    ${groupid2}    ${groupid3}
    ...    ${groupid4}    ${header}    ${timeout}
    [Documentation]    获取多个群详情
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid1},${groupid2},${groupid3},${groupid4}
    Run Keyword And Return    Get Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}
    [Arguments]    ${method}    ${session}    ${org_name}    ${app_name}    ${groupid}    ${header}
    ...    ${data}    ${timeout}
    [Documentation]    1、获取单个群详情
    ...    2、转让群组
    ...    3、修改群组信息
    ...    4、删除群组
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}
    Run Keyword And Return if    ${method}=='get'    Get Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}
    Run Keyword And Return if    ${method}=='put'    Put Request    ${session}    ${uri}    headers=${header}    data=${data}
    ...    timeout=${timeout}
    Run Keyword And Return if    ${method}=='delete'    delete Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/users/{username}
    [Arguments]    ${method}    ${session}    ${org_name}    ${app_name}    ${groupid}    ${username}
    ...    ${header}    ${timeout}
    [Documentation]    1、添加单个群组成员
    ...    2、删除单个群组成员
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/users/${username}
    Run Keyword And Return if    ${method}=='post'    Post Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}
    Run Keyword And Return if    ${method}=='delete'    delete Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/admin
    [Arguments]    ${method}    ${session}    ${org_name}    ${app_name}    ${groupid}    ${header}
    ...    ${data}    ${timeout}
    [Documentation]    1、添加群管理员
    ...    2、获取群管理员列表
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/admin
    Run Keyword And Return if    ${method}=='get'    Get Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}
    Run Keyword And Return if    ${method}=='post'    Post Request    ${session}    ${uri}    headers=${header}    data=${data}
    ...    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/users?
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${groupid}    ${header}    ${timeout}
    [Documentation]    获取群组成员
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/users
    Run Keyword And Return    Get Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/admin/{adminname}
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${groupid}    ${adminname}    ${header}
    ...    ${timeout}
    [Documentation]    移除群管理员
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/admin/${adminname}
    Run Keyword And Return    delete Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/users
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${groupid}    ${header}    ${data}
    ...    ${timeout}
    [Documentation]    批量添加群组成员
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/users
    Run Keyword And Return    Post Request    ${session}    ${uri}    headers=${header}    data=${data}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/users/{memb1},{memb2},{memb3}
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${groupid}    ${memb1}    ${memb2}
    ...    ${memb3}    ${header}    ${timeout}
    [Documentation]    批量删除群组成员
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/users/${memb1},${memb2},${memb3}
    Run Keyword And Return    delete Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/blocks/users/{username}
    [Arguments]    ${method}    ${session}    ${org_name}    ${app_name}    ${groupid}    ${username}
    ...    ${header}    ${timeout}
    [Documentation]    1、添加单个用户到黑名单
    ...    2、从黑名单移除单个用户
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/blocks/users/${username}
    Run Keyword And Return if    ${method}=='delete'    delete Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}
    Run Keyword And Return if    ${method}=='post'    Post Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/blocks/users
    [Arguments]    ${method}    ${session}    ${org_name}    ${app_name}    ${groupid}    ${header}
    ...    ${data}    ${timeout}
    [Documentation]    1、添加批量用户到群组黑名单
    ...    2、获取群组黑名单列表
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/blocks/users
    Run Keyword And Return if    ${method}=='get'    Get Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}
    Run Keyword And Return if    ${method}=='post'    Post Request    ${session}    ${uri}    headers=${header}    data=${data}
    ...    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/blocks/users/{blockuser1}{blockuser2}{blockuser3}
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${groupid}    ${blockuser1}    ${blockuser2}
    ...    ${blockuser3}    ${header}    ${timeout}
    [Documentation]    从群组黑名单中批量移除用户
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/blocks/users/${blockuser1},${blockuser2},${blockuser3}
    Run Keyword And Return    delete Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/apply
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${groupid}    ${header}    ${timeout}
    [Documentation]    用户申请加入群组
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/apply
    ${joingrpBody}    set variable    {"message":"join group"}
    Run Keyword And Return    Post Request    ${session}    ${uri}    headers=${header}    data=${joingrpBody}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/apply_verify
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${groupid}    ${header}    ${data}
    ...    ${timeout}
    [Documentation]    审批申请加入群组的用户
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/apply_verify
    Run Keyword And Return    Post Request    ${session}    ${uri}    headers=${header}    data=${data}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/quit
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${groupid}    ${header}    ${timeout}
    [Documentation]    离开群组
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/quit
    Run Keyword And Return    delete Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/mute
    [Arguments]    ${method}    ${session}    ${org_name}    ${app_name}    ${groupid}    ${header}
    ...    ${data}    ${timeout}
    [Documentation]    1、禁言群成员
    ...    2、获取禁言成员列表
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/mute
    Run Keyword And Return if    ${method}=='get'    Get Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}
    Run Keyword And Return if    ${method}=='post'    Post Request    ${session}    ${uri}    headers=${header}    data=${data}
    ...    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/mute/{user}
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${groupid}    ${user}    ${header}
    ...    ${timeout}
    [Documentation]    解除禁言
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/mute/${user}
    Run Keyword And Return    delete Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}

/{org_name}/{app_name}/chatgroups/{groupid}/share_files
    [Arguments]    ${session}    ${org_name}    ${app_name}    ${groupid}    ${header}    ${timeout}
    [Documentation]    获取群共享文件列表
    ${uri}=    set variable    /${org_name}/${app_name}/chatgroups/${groupid}/share_files
    Run Keyword And Return    Get Request    ${session}    ${uri}    headers=${header}    timeout=${timeout}
